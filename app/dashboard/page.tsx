...
// Location: latise/app/dashboard/page.tsx
// Home view — mirrors Coinbase home exactly:
//   Left: total shielded value + asset rows + "For you" cards
//   Right: Shield/Unshield quick panel + action shortcuts
"use client";
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 min-w-0 overflow-y-auto border-r border-gray-200">
        <div className="p-6">
          ...
          {/* Asset rows — like Coinbase's Crypto + Cash rows */}
...
function PairsTable({
  pairs, network, loading,
}: {
  pairs: ReturnType<typeof useRegistry>["data"] extends undefined ? never[] : NonNullable<ReturnType<typeof useRegistry>["data"]>;
  network: Network;
  loading: boolean;
}) {
  if (loading) {
...
function RightPanel({
  network, authenticated, login, address,
}: {
  network: Network;
  ...
  return (
    <div
      className="shrink-0 overflow-y-auto border-l border-gray-200 bg-white px-5 py-6 space-y-5"
      style={{ width: 300 }}
    >
      ...
      <div className="space-y-1 pt-2 border-t border-gray-100">
        <QuickAction
          icon="↑"
          label="Shield assets"
          href={`/dashboard/vault?tab=shield&network=${network}`}
          color="bg-[#d0ede2] text-[#156640]"
        />
        <QuickAction
          icon="↓"
          label="Unshield assets"
          href={`/dashboard/vault?tab=unshield&network=${network}`}
          color="bg-[#d0ede2] text-[#156640]"
        />
        <QuickAction
          icon="🔍"

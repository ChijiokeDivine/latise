  ...
  const rows: TxRow[] = [
    ...wrapEvents.map((e) => {
      const pair = pairs.find((p) => 
        p.wrapperAddress.toLowerCase() === e.wrapperAddress.toLowerCase()
      );
      return {
        type: "Wrap" as const,
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Recent wrap and unwrap activity on {network}
        </p>
      </div>
            ...
            {isLoading ? (
              ...
              rows.map((row, i) => (
                <tr key={`${row.txHash}-${i}`} className="hover:bg-gray-50/50 transition">
                  ...
                  <td className="py-3 px-4 font-medium text-gray-900 text-[13px]">
                    {row.token}
                  </td>
